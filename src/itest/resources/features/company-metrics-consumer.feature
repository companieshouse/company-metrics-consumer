Feature: Process Company Metrics Consumer information
  ...
  Scenario Outline: Processing invalid message
    ...
    Examples:
      | avroMessage         | KafkaTopic             | KafkaInvalidTopic                                       | statusCode | times |
      | non_avro_msg_1.avro | stream-company-charges | stream-company-charges-company-metrics-consumer-invalid | 200        | 0     |
  ...
  Scenario Outline: Processing valid avro message with invalid resource uri
    ...
    And Stubbed Company Metrics API should be called "<times>" times
    Examples:
      | companyNumber | resourceUriFormat      | KafkaTopic             | KafkaInvalidTopic                                       | statusCode | times |
      | 12345678      | /companyabc/%s/charges | stream-company-charges | stream-company-charges-company-metrics-consumer-invalid | 200        | 0     |
  ...
  Scenario Outline: Processing valid avro message and backend api returning 400 bad request error
    ...
    Examples:
      | companyNumber | resourceUriFormat   | KafkaTopic             | KafkaInvalidTopic                                       | statusCode | times |
      | 12345678      | /company/%s/charges | stream-company-charges | stream-company-charges-company-metrics-consumer-invalid | 400        | 1     |
